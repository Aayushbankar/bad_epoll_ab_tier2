...
A critical open question remains whether the race is naturally schedulable without debugger assistance. Static and runtime analyses confirmed that `cond_resched()` near the race window is a no-op due to `PREEMPT_VOLUNTARY` and dynamic static keys (VER-035, [NAT-002_RESULTS.md](tier2/evidence/NAT-002/NAT-002_RESULTS.md)). The true race window is extremely narrow, purely instruction-timing limited to approximately 250-550 cycles. Under a closed-loop sweep within QEMU, we achieved 0 hits in 102,740 attempts (VER-039, [NAT-005_raw_serial.log](tier2/evidence/NAT-005_raw_serial.log)). We additionally tested this baseline against a full KASLR-enabled environment (AND-002) using a statically-compiled GDB memory-hooking strategy; KASLR did not alter the timing dynamics and also produced 0 hits over 92,740 iterations (VER-042, [AND-002_RESULTS.md](tier2/evidence/AND-002_RESULTS.md)). This is a legitimate negative result under QEMU's TCG software emulation, but we acknowledge the honest environmental limitation: TCG does not model real cache-coherency or memory-bus timing characteristics of physical ARM64 chips.
