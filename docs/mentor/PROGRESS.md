...
5. **Exact stale access that KASAN detects**: ~~Thread A resumes and executes `hlist_del_rcu(&epi->fllink)`, which attempts a doubly-linked list removal write on the freed memory space.~~ [Corrected 2026-07-24, see EVO-005 in VERIFICATION_LEDGER.md: the stale write is `list_del_init(&epi->rdllink)` at offsets 24 and 32 of the freed `epitem`, not `hlist_del_rcu(&epi->fllink)`]
6. **Exact allocation cache, object size, and stale-access offset supported by existing evidence**: ~~The `eventpoll` struct (176 bytes) is allocated in the `kmalloc-192` cache. KASAN confirms the stale write is exactly 160 bytes (`0xa0`) into the 192-byte region, perfectly aligning with the `refs` list head.~~ [Corrected 2026-07-24, see EVO-005 in VERIFICATION_LEDGER.md: the UAF victim is `struct epitem` (120 bytes) in the dedicated `eventpoll_epi` slab cache (created with SLAB_HWCACHE_ALIGN|SLAB_ACCOUNT); the stale write (`list_del_init(&epi->rdllink)`) targets offsets 24 and 32 of the freed epitem]
7. **All existing KASAN evidence**: A synchronous HW_TAGS MTE KASAN report is captured confirming an invalid-access write at `[fc]` vs freed memory tag `[fe]` on the ~~`kmalloc-192`~~ `eventpoll_epi` slab object by `cve_2026_46242_` (PID 63) on CPU 0. [Corrected 2026-07-24, see EVO-005 in VERIFICATION_LEDGER.md: the freed object is `struct epitem` in the dedicated `eventpoll_epi` cache, not a generic `kmalloc-192` object]
8. **Every claim that is verified**: 
   - The UAF race window exists in `__ep_remove` between `f_ep = NULL` and ~~`hlist_del_rcu`~~ `list_del_init(&epi->rdllink)`. [Corrected 2026-07-24, see EVO-005 in VERIFICATION_LEDGER.md: stale write is `list_del_init`, not `hlist_del_rcu`]
   - The lockless fast-path in `eventpoll_release` is reachable and allows Thread B to bypass locks.
   - ~~The UAF write deterministically targets the freed `inner_epoll` memory region.~~ [Corrected 2026-07-24, see EVO-005 in VERIFICATION_LEDGER.md: the UAF write targets the freed `epitem` (in the `eventpoll_epi` dedicated cache), not the freed `inner_epoll`]
9. **Every claim that is not verified**: 
   ...
   - Ability to reliably groom/reallocate the ~~`kmalloc-192`~~ `eventpoll_epi` cache object concurrently outside of a mechanically paused GDB state. [Corrected 2026-07-24, see EVO-005 in VERIFICATION_LEDGER.md: victim object is in the dedicated `eventpoll_epi` cache, not `kmalloc-192`]
