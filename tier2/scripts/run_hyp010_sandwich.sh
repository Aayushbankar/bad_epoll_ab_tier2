#!/bin/bash
# run_hyp010_sandwich.sh — HYP-010 Part B: Victim-Sandwich Reclaim Execution
set -euo pipefail
...
echo "============================================================"
echo "HYP-010 Part B: Victim-Sandwich Reclaim (Forced Chain)"
...
# 2. Compile harness
echo "[2/4] Compiling test_hyp010_sandwich..."
${CROSS} -static -O0 -g -o "${TIER2_DIR}/rootfs/harness" "${TIER2_DIR}/scripts/test_hyp010_sandwich.c" \
...
DEBUG=1 ./scripts/run_qemu.sh > qemu_hyp010_sandwich.log 2>&1 &
QEMU_PID=$!
...
gdb -batch -q -x "${TIER2_DIR}/scripts/exp_hyp010_gdb.py" "${TIER2_DIR}/android/artifacts/vmlinux" 2>&1 || true
...
if [ -f "${TIER2_DIR}/qemu_serial.log" ]; then
    cp "${TIER2_DIR}/qemu_serial.log" "${EVIDENCE_DIR}/HYP-010_sandwich_serial.log" 2>/dev/null || true
fi
