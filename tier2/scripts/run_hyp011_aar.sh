#!/bin/bash
# run_hyp011_aar.sh — HYP-011: dma-buf AAR Execution
set -euo pipefail
...
echo "============================================================"
echo "HYP-011 Part 1: dma-buf Reclaim Vehicle & AAR Verification"
echo "Kernel: GKI 6.1.23 + CONFIG_DMABUF_HEAPS_SYSTEM=y (nokaslr, QEMU TCG)"
echo "============================================================"
...
DEBUG=1 ./scripts/run_qemu.sh > qemu_hyp011_aar.log 2>&1 &
QEMU_PID=$!
...
if [ -f "${TIER2_DIR}/qemu_serial.log" ]; then
    cp "${TIER2_DIR}/qemu_serial.log" "${EVIDENCE_DIR}/HYP-011_part1_serial.log" 2>/dev/null || true
fi
