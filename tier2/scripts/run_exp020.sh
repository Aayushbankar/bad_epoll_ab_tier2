...
cd "$TIER2_DIR"
./aarch64-linux-musl-cross/bin/aarch64-linux-musl-gcc -static -O0 -g -o rootfs/harness scripts/test_exp020.c -pthread
...
echo "[*] Launching QEMU..."
DEBUG=1 ./scripts/run_qemu.sh > qemu_out.log 2>&1 &
QEMU_PID=$!

echo "[*] QEMU launched with PID $QEMU_PID"
sleep 2

echo "[*] Running EXP-020 GDB script..."
gdb -batch -q -x scripts/exp020_gdb.py android/artifacts/vmlinux
