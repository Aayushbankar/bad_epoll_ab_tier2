# exp_hyp011_aar_gdb.py — HYP-011: GDB Automation for dma-buf AAR & Live Re-Aim
import gdb
import time
import os
...
gdb.execute("set logging file evidence/HYP-011/HYP-011_part1_raw_gdb.log")
...
class EpShowFdinfoBreakpoint(gdb.Breakpoint):
    ...
    def stop(self):
        ...
        log("-----------------------------------------------------------------")
        log(f"[HYP-011] Hit ep_show_fdinfo (invocation #{g_fdinfo_count})")
        ...
        try:
            ...
            if rb_root != 0:
                ...
                # Dump 32 bytes of the victim struct file slot
                try:
                    slot_bytes = gdb.execute(f"x/4gx {epi_file}", to_string=True).strip()
                    log(f"    slot dump:\n{slot_bytes}")
