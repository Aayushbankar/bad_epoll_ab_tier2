...
| VER-016 | 2026-07-28 | EXP-008 Timing Analysis: The `list_del_init` corrupting write on `epi->rdllink` executes *before* `call_rcu(epi)` in `__ep_remove`. Therefore, the memory is corrupted before it is freed. When reclaimed via `epoll_ctl(EPOLL_CTL_ADD)`, the newly allocated `struct epitem` is fully initialized with zeroed or valid data, completely overwriting the `LIST_POISON` values. The corruption does not land on a live reclaimed object. | `__ep_remove` | RUNTIME (GDB trace) | tier2/evidence/EXP-008_raw_gdb.log | **VERIFIED** |
