...
| HYP-002 | 2026-09-05 | Kernel-Side Debugfs Counter Ground-Truth Test: Add debug counters in `__ep_remove`/`ep_free` to detect if natural race fires silently without oracle detecting it | test_hyp002.c | (none) | run_hyp002.sh, run_hyp002_repro.sh | **INCONCLUSIVE (GKI 6.1.23)** / **REJECTED (6.12.67)**: Race detected once on GKI 6.1.23 (cf24bd6), but not reproducible in 15,000 additional iterations (1/20,000 total; ~0.005%). Detection mechanism is sound (kzalloc zeroes memory), but race is extremely rare under QEMU TCG. | `tier2/evidence/HYP-002/HYP-002_RESULTS.md`, `tier2/evidence/HYP-002/HYP-002_RESULTS_GKI.md` |
