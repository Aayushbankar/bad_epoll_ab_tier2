...
| HYP-002 | 2026-09-05 | Kernel-Side Debugfs Counter Ground-Truth Test: Add debug counters in `__ep_remove`/`ep_free` to detect if natural race fires silently without oracle detecting it | test_hyp002.c | (none) | run_hyp002.sh, run_hyp002_repro.sh, run_hyp002_batch20k.sh | **INCONCLUSIVE (GKI 6.1.23)** / **REJECTED (6.12.67)**: Race detected once on GKI 6.1.23 (cf24bd6), but not reproducible across 35,000 additional iterations (1/40,000 total; ~0.0025%, 95% CI upper bound <0.00857%). LOW-CONFIDENCE finding. | `tier2/evidence/HYP-002/HYP-002_RESULTS.md`, `tier2/evidence/HYP-002/HYP-002_RESULTS_GKI.md` |
| HYP-001 | 2026-09-06 | Timerfd Interrupt Widening Characterization: Instrument actual interrupt delivery latency/jitter under QEMU TCG to determine if TCG timing-fidelity specifically explains race non-reproduction | test_hyp001_timerfd.c | (none) | run_hyp001.sh | **CONFIRMED**: Target 1us exhibits 85.58us floor and 256.69us mean (256.7x expansion); storm coalesces 88,164 ticks/wakeup with only 203 IRQs delivered; proves NAT-001 failure is TCG artifact | `tier2/evidence/HYP-001/HYP-001_RESULTS.md`, `tier2/evidence/HYP-001/HYP-001_raw_serial.log` |
| EXP-027 | 2026-09-06 | Page-Level struct file Cross-Cache Reclaim: Assess DMA_HEAP availability and test mass-freeing struct file via fork-holding to drain filp slab pages to buddy allocator | test_filp_drain.c | (none) | run_exp027.sh | **CONFIRMED (Drain)** / **INACTIVE (DMA_HEAP)**: Mass-freeing 32,000 files drained 1,991/2,000 slabs back to buddy (99.55% efficiency); however `CONFIG_DMABUF_HEAPS_SYSTEM` is not set on current GKI 6.1.23 build | `tier2/evidence/EXP-027/EXP-027_RESULTS.md`, `tier2/evidence/EXP-027/EXP-027_raw_serial.log` |
| EXP-016b | 2026-09-06 | kmalloc-192 Repeated-Reset Gadget Candidate Search: Audit kmalloc-192 structs where offset 160 is a counter, flag, or small-integer field for repeated-reset gadgets | audit_kmalloc192_reset.py | (none) | (python) | **COMPLETED (Negative)**: 431 structs analyzed; surfaced binder_device->ref (refcount_t saturation), fib4_rule->srcmask (requires CAP_NET_ADMIN), x509_certificate->blacklisted (transient); no viable unprivileged LPE primitive found | `tier2/evidence/EXP-016b/EXP-016b_RESULTS.md`, `tier2/evidence/EXP-016b/audit_gki_candidates.json` |
